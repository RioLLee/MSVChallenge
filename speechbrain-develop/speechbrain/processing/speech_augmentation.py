...
# added
class SpeedPitchPerturb(torch.nn.Module):
   def __init__(self, ori_freq,
                speed=[0.9,1.0,1.1],
                perturb_prob = 1.0,
                pitch = False):
       super(SpeedPitchPerturb, self).__init__()
       self.pitch = pitch
       self.speeds = speed
       self.ori_freq = ori_freq
       self.perturb_prob  = perturb_prob
   def forward(self, waveform):

       starttime = time.time()
       sample_rate = self.ori_freq
       if torch.rand(1) > self.perturb_prob:
          return waveform

       hyparam = 'speed' if self.pitch else 'tempo'
       #print("hyparam--->",hyparam)
       speed_index = torch.randint(len(self.speeds),(1,))[0]

       #print("---> speed ",  str(self.speeds[speed_index]))

       waveform_output, sr = sox_effects.apply_effects_tensor(waveform.cpu(),
       sample_rate,
       [[hyparam, str(self.speeds[speed_index])], ['rate', str(self.ori_freq)]]
       )
       wavform_ = waveform_output.to(waveform.device)

       endtime = time.time()
       #print("Time of torch.sox.tempo:",(endtime-starttime))
       return waveform_output.to(waveform.device)
