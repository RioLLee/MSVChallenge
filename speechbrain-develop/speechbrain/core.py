...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        self.opt_disc = None
        if 'opt_disc' in hparams:
            self.opt_disc = hparams['opt_disc']
        ...
        self.domain_critic = None
        if 'domain_critic' in hparams:
            self.domain_critic = hparams['domain_critic'].to(self.device)
    ...
    def init_optimizers(self):
        ...
        if self.opt_disc is not None:
            self.optimizer_disc = self.opt_disc(self.domain_critic.parameters())

            if self.checkpointer is not None:
                self.checkpointer.add_recoverable("optimizer_disc", self.optimizer_disc)
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs = self.compute_forward(batch, Stage.TRAIN)
            loss = self.compute_objectives(outputs, batch, Stage.TRAIN)
            if hasattr(self, "train_step") and self.train_step != 0:
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            if self.domain_critic is not None:
                self.domain_critic.train()
                
            if hasattr(self.modules, "teacher_model") :
                self.modules.teacher_model.eval()
