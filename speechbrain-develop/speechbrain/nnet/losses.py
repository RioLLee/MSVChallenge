...
class CCE(nn.Module):
    def __init__(self, device='cuda:0', balancing_factor=1):
        super(CCE, self).__init__()
        self.nll_loss = nn.NLLLoss()
        self.device = device # {'cpu', 'cuda:0', 'cuda:1', ...}
        self.balancing_factor = balancing_factor

        self.aam_softmax = LogSoftmaxWrapper(AdditiveAngularMargin(margin=0.2, scale=30))

    def forward(self, yHat, y, length=None):
        # Note: yHat.shape[1] <=> number of classes

        # cross entropy
        # cross_entropy = self.nll_loss(F.log_softmax(yHat, dim=1), y)
        cross_entropy = self.aam_softmax(yHat, y)
        ...
        complement_entropy = torch.sum(output) / (float(batch_size))

        logger.info(f"AAM_loss---> {cross_entropy}")
        logger.info(f"CEloss---> {complement_entropy}")
        logger.info(f"CCE_loss---> {cross_entropy - self.balancing_factor * complement_entropy}")
